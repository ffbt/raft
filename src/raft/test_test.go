...
func TestReElection2A(t *testing.T) {
	...
	DPrintf(1, "connect leader1")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	DPrintf(1, "disconnect leader2 and leader2+1")
	...
	DPrintf(1, "connect leader2+1")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	DPrintf(1, "connect leader2")
	cfg.connect(leader2)
	cfg.checkOneLeader()
